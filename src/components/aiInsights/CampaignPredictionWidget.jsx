...
5. Best performing campaign pattern identified`,
  ...
  const trendColors = { improving: 'bg-emerald-900/30 text-emerald-400', stable: 'bg-blue-900/30 text-blue-400', declining: 'bg-red-900/30 text-red-400' };
          ...
          <div className="space-y-4">
            ...
            {results && (
              <div className="space-y-3">
                ...
                <p className="text-xs text-slate-400 bg-violet-900/20 rounded-lg px-3 py-2 leading-relaxed border border-violet-500/30">{results.health_assessment}</p>
                {results.best_pattern && (
                  <p className="text-[10px] text-violet-600 italic">🏆 Best pattern: {results.best_pattern}</p>
                )}
                {results.recommendations?.length > 0 && (
                  <div className="bg-amber-900/20 rounded-lg p-2.5 border border-amber-500/30">
