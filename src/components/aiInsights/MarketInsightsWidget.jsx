...
const TYPE_CFG = {
  growth: { icon: TrendingUp, bg: 'bg-emerald-900/20 border-emerald-500/30', text: 'text-emerald-400' },
  threat: { icon: ShieldAlert, bg: 'bg-red-900/20 border-red-500/30', text: 'text-red-400' },
  opportunity: { icon: Target, bg: 'bg-blue-900/20 border-blue-500/30', text: 'text-blue-400' },
  action: { icon: Zap, bg: 'bg-violet-900/20 border-violet-500/30', text: 'text-violet-700' },
};
const PRIO = { high: 'bg-red-900/30 text-red-400', medium: 'bg-amber-900/30 text-amber-400', low: 'bg-slate-700 text-slate-400' };
      ...
      <CardContent>
        ...
        {results && (
          <div className="space-y-3">
            <div className="flex items-center justify-between bg-gradient-to-r from-slate-800/40 to-emerald-900/20 rounded-lg p-3 border border-emerald-500/30">
              <p className="text-xs text-slate-300 leading-relaxed flex-1">{results.summary}</p>
              {results.market_health_score != null && (
                <Badge className={`ml-3 shrink-0 text-xs ${results.market_health_score >= 70 ? 'bg-emerald-900/30 text-emerald-400' : results.market_health_score >= 40 ? 'bg-amber-900/30 text-amber-400' : 'bg-red-900/30 text-red-400'}`}>
