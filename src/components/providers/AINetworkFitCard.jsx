...
6. Predicted referral volume if engaged`,
  ...
  const scoreBg = (s) => s >= 75 ? 'bg-emerald-900/15' : s >= 50 ? 'bg-amber-900/15' : 'bg-red-900/15';
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between bg-slate-800/40 rounded-lg p-2">
              <div className="flex items-center gap-2">
                ...
                <Badge className={`text-[9px] ${analysis.overall_priority === 'high' ? 'bg-emerald-900/15 text-emerald-400' : analysis.overall_priority === 'medium' ? 'bg-amber-900/15 text-amber-400' : 'bg-slate-700 text-slate-400'}`}>
              ...
              <div key={section.label}>
                ...
                <ul className="space-y-0.5">
                  {section.items.slice(0, 3).map((r, i) => (
                    <li key={i} className="text-[11px] text-slate-300 pl-3 relative before:content-['•'] before:absolute before:left-0 before:text-slate-400">{r}</li>
                  ))}
                </ul>
              </div>
            ))}
