...
export default function NPPESImportResults({ result }) {
  ...
  const stats = [
    { label: 'Total Fetched', value: result.total_fetched, color: 'text-white' },
    { label: 'Valid Records', value: result.valid_rows, color: 'text-green-400' },
    ...
    <Card className="border-green-500/30">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {stats.map(s => (
            <div key={s.label} className="p-3 bg-slate-800/40 rounded-lg text-center">
