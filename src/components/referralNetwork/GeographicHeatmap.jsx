...
function getHeatColor(intensity) {
  if (intensity >= 0.8) return { bg: 'bg-red-900/30', text: 'text-red-400', border: 'border-red-500/30' };
  if (intensity >= 0.6) return { bg: 'bg-orange-900/25', text: 'text-orange-400', border: 'border-orange-500/25' };
  if (intensity >= 0.4) return { bg: 'bg-amber-900/20', text: 'text-amber-400', border: 'border-amber-500/20' };
  if (intensity >= 0.2) return { bg: 'bg-cyan-900/15', text: 'text-cyan-400', border: 'border-cyan-500/15' };
          ...
          <div className="flex gap-1">
            {[
              ...
              <button
                ...
                className={`text-[10px] px-2 py-0.5 rounded ${metric === m.key ? 'bg-cyan-900/20 text-cyan-400' : 'text-slate-500 hover:text-slate-300'}`}
              >
                {m.label}
              </button>
            ))}
      ...
      <CardContent>
        ...
        <div className="flex items-center justify-center gap-2 mt-3">
          ...
          <div className="flex gap-0.5">
            {['bg-slate-700/30', 'bg-cyan-900/15', 'bg-amber-900/20', 'bg-orange-900/25', 'bg-red-900/30'].map((c, i) => (
