...
function HubCard({ hub, rank }) {
  return (
    <div className="flex items-center gap-3 p-2.5 rounded-lg border border-slate-700/30 hover:bg-slate-800/30">
      <div className="w-7 h-7 rounded-full bg-amber-900/200/15 flex items-center justify-center text-amber-400 text-xs font-bold shrink-0">
        {rank}
      </div>
      ...
      <Badge className={`text-[9px] shrink-0 ${hub.hubScore >= 80 ? 'bg-red-900/200/15 text-red-400' : hub.hubScore >= 50 ? 'bg-amber-900/200/15 text-amber-400' : 'bg-slate-700/50 text-slate-400'}`}>
        {hub.hubScore}
      </Badge>
    </div>
  );
  ...
  return (
    <div className="p-2.5 rounded-lg border border-slate-700/30 hover:bg-slate-800/30">
      <div className="flex items-center justify-between mb-1.5">
        ...
        <Badge className={`text-[9px] ${gap.severity === 'high' ? 'bg-red-900/200/15 text-red-400' : 'bg-amber-900/200/15 text-amber-400'}`}>
          {gap.severity}
        </Badge>
      </div>
      ...
      {gap.missingSpecialties?.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-1.5">
          {gap.missingSpecialties.slice(0, 4).map(s => (
            <Badge key={s} className="bg-red-900/200/10 text-red-400 text-[8px] border border-red-500/20">{s}</Badge>
          ))}
        </div>
      )}
    </div>
