    ...
    <Card>
      ...
      <CardContent className="space-y-1.5 max-h-96 overflow-y-auto">
        ...
        {sorted.map(report => {
          ...
          return (
            <div key={report.id} className="flex items-center gap-2 p-2 rounded-lg border hover:bg-slate-800/60 transition-colors group">
              <ChartIcon className="w-4 h-4 text-slate-400 flex-shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-slate-300 truncate">{report.name}</p>
