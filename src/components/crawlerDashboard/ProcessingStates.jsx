        ...
        {!isActive && activeItems.length === 0 && processingStatesCodes.length === 0 ? (
          ...
          <div className="space-y-2">
            {activeItems.map(batch => {
              ...
              return (
                <div key={batch.id} className="flex items-center justify-between p-3 bg-blue-900/200/10 rounded-lg border border-blue-500/20">
            ...
            {processingStatesCodes.filter(st => !activeItems.some(b => getStateFromFileName(b.file_name) === st)).map(st => (
              <div key={st} className="flex items-center gap-3 p-3 bg-blue-900/200/5 rounded-lg border border-blue-500/10">
