        ...
        <div className="space-y-4 py-2">
          ...
          <div className="bg-gray-50 rounded-lg p-3 text-sm space-y-1">
            ...
            <div className="flex gap-4 mt-1 text-xs text-gray-500">
              ...
              {batch.valid_rows > 0 && <span className="text-green-600">Valid: {batch.valid_rows?.toLocaleString()}</span>}
              {batch.imported_rows > 0 && <span className="text-blue-600">Imported: {batch.imported_rows?.toLocaleString()}</span>}
              {batch.invalid_rows > 0 && <span className="text-red-600">Invalid: {batch.invalid_rows?.toLocaleString()}</span>}
            </div>
            {batch.retry_count > 0 && (
              <p className="text-amber-600 text-xs mt-1">Previously retried {batch.retry_count} time(s)</p>
            )}
          </div>
          ...
          <Tabs value={retryMode} onValueChange={setRetryMode}>
            <TabsList className="grid grid-cols-4 h-8">
              ...
              <TabsTrigger value="row_range" className="text-xs">Row Range</TabsTrigger>
              <TabsTrigger value="failed_only" className="text-xs">Failed Only</TabsTrigger>
              <TabsTrigger value="resume" className="text-xs" disabled={!batch.imported_rows}>Resume</TabsTrigger>
            </TabsList>
          ...
          <div className="space-y-2">
            ...
            {npiFilter !== null && npiFilter !== undefined && (
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <Label className="text-xs">Filter by NPIs (comma-separated)</Label>
                  <Input
                    value={npiFilter || ''}
          ...
          <div className="space-y-2 border-t pt-3">
            ...
            <div className="flex items-center justify-between">
              <div>
                <Label className="text-sm">Skip validation</Label>
                <p className="text-xs text-gray-400">Import directly without re-validating rows</p>
              </div>
