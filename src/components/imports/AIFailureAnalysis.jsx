...
Be specific and actionable. Reference actual error details from the samples.`,
  ...
  const severityColors = {
    ...
    high: 'bg-orange-900/15 text-orange-400 border-orange-500/20',
    critical: 'bg-red-900/15 text-red-400 border-red-500/20',
  };
    ...
    <Card className="bg-[#141d30] border-purple-500/20">
      ...
      {(expanded || !compact) && (
        <CardContent className="space-y-3">
          {loading && (
            <div className="flex items-center gap-3 p-4 bg-purple-900/5 rounded-lg border border-purple-500/10">
                    ...
                    <div key={idx} className="flex items-start gap-2 p-2 bg-slate-800/30 rounded border border-slate-700/30">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          ...
                          {rec.automated && (
                            <Badge className="text-[8px] bg-emerald-900/10 text-emerald-400 border-emerald-500/20">auto-fixable</Badge>
                          )}
                        </div>
                      </div>
                    </div>
                ...
                <div className="bg-slate-800/50 rounded-lg p-3 border border-slate-700/40">
                      ...
                      <div className="flex-1">
                        ...
                        {analysis.suggested_retry?.mode && analysis.suggested_retry.mode !== 'none' && (
                          <div className="flex flex-wrap gap-2 mt-2">
                            ...
                            {analysis.suggested_retry.row_offset > 0 && <Badge className="text-[9px] bg-amber-900/10 text-amber-400 border-amber-500/20">offset: {analysis.suggested_retry.row_offset}</Badge>}
                          </div>
                        )}
                      </div>
                  ...
                  {analysis.resolution_path === 'schema_change' && (
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <Button onClick={() => window.location.href = '/import-monitoring?tab=rules'} size="sm" variant="outline" className="shrink-0 h-8 text-xs border-amber-500/30 text-amber-400 hover:bg-amber-900/10">
                  ...
                  {analysis.resolution_path === 'manual_file_cleanup' && (
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <Button onClick={() => window.open(batch.file_url, '_blank')} size="sm" variant="outline" className="shrink-0 h-8 text-xs border-blue-500/30 text-blue-400 hover:bg-blue-900/10">
              ...
              {!analysis.resolution_path && (
                <>
                  ...
                  {analysis.can_auto_fix && analysis.suggested_retry?.mode !== 'none' && (
                    <div className="bg-cyan-900/5 border border-cyan-500/20 rounded-lg p-3">
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap gap-2 mt-1.5">
                            ...
                            {analysis.suggested_retry.dry_run_first && (
                              <Badge className="text-[9px] bg-blue-900/10 text-blue-400 border-blue-500/20">dry run first</Badge>
                            )}
                            {analysis.suggested_retry.sheet_filter && (
                              <Badge className="text-[9px] bg-violet-900/10 text-violet-400 border-violet-500/20">
                                sheet: {analysis.suggested_retry.sheet_filter}
                              </Badge>
                            )}
                            {analysis.suggested_retry.row_offset > 0 && (
                              <Badge className="text-[9px] bg-amber-900/10 text-amber-400 border-amber-500/20">
                  ...
                  {!analysis.can_auto_fix && (
                    <div className="bg-amber-900/5 border border-amber-500/15 rounded-lg p-2.5 text-[11px] text-amber-400/80 flex items-start gap-2">
