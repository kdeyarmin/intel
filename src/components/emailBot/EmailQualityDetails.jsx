...
export default function EmailQualityDetails({ analysis, _email, compact = false }) {
  ...
  if (compact) {
    ...
    return (
      <div className="flex items-center gap-2 pt-1.5">
        <Badge className={
          analysis.score >= 75 ? 'bg-emerald-900/15 text-emerald-400 border-emerald-500/20' :
          analysis.score >= 50 ? 'bg-amber-900/15 text-amber-400 border-amber-500/20' :
  ...
  const getConfidenceColor = (confidence) => {
    switch (confidence) {
      case 'high': return 'bg-emerald-900/15 text-emerald-400 border-emerald-500/20';
      case 'medium': return 'bg-amber-900/15 text-amber-400 border-amber-500/20';
      case 'low': return 'bg-red-900/15 text-red-400 border-red-500/20';
