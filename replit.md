...
- **Facility Pages:** Unified detail pages for Hospitals, Home Health Agencies, Hospices, and Nursing Homes/SNFs. Each type has a searchable list page (with state filter) and a detail page consolidating all imported CMS data per facility. Backend: `server/functions/facilityDetail.ts` (getFacilityDetail, listFacilities). Pages: `Hospitals.jsx`, `HomeHealthAgencies.jsx`, `Hospices.jsx`, `NursingHomes.jsx`, `FacilityDetail.jsx`. DB indexes: `idx_medfac_facility_type`, `idx_medfac_provider_id`.
