  ...
  try {
    ...
    if (!provider.email) {
      return Response.json({ 
        success: false, 
        message: 'Provider has no email to analyze' 
      });
    }

    // Analyze the email
    const analysis = analyzeEmail(provider.email);

    // Update provider with analysis
    await base44.asServiceRole.entities.Provider.update(provider_id, {
      email_quality_score: analysis.score,
      email_quality_confidence: analysis.confidence,
      email_quality_reasons: analysis.reasons,
      email_quality_risk_flags: analysis.riskFlags,
      email_quality_analysis: analysis.analysis,
