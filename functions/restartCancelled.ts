...
Deno.serve(async (req) => {
    ...
    let skip = 0;
    while(hasMore) {
        const res = await base44.asServiceRole.entities.ImportBatch.filter({ status: 'cancelled' }, '-created_date', 50, skip);
        allCancelled.push(...res);
    ...
    for (const batch of allCancelled) {
        try {
