...
// Instead of relying on an incomplete zip prefix map, we query the NPPES API
// directly by state. The API supports state-level queries. We use zip prefixes
// ONLY as a fallback for deep expansion when result counts exceed the skip limit.
// To get ALL providers, we paginate by enumeration_type and use the postal_code
// wildcard with 2-digit prefixes 00-99 to partition large states.
const ALL_ZIP_PREFIXES = [];
for (let i = 0; i <= 99; i++) ALL_ZIP_PREFIXES.push(String(i).padStart(2, '0'));
    ...
    try {
        ...
        if (effectiveAction === 'status') {
            ...
            for (const [st, b] of Object.entries(stateLatest)) {
                ...
                else processingStates.push(st); // processing or validating
            }
